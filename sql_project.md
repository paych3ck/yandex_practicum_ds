...
SELECT activity, ROUND(AVG(investment_rounds)) AS average_investment_rounds
FROM (
    SELECT *,
        CASE
...
SELECT country_code, 
    MIN(invested_companies) as minc,
    MAX(invested_companies) as maxc,
...
FROM company AS comp
JOIN funding_round AS fr ON comp.id=fr.company_id
WHERE comp.status='closed'
...
FROM people AS pe
JOIN company AS comp ON pe.company_id=comp.id
WHERE pe.company_id IN (SELECT DISTINCT comp.id
FROM company AS comp
JOIN funding_round AS fr ON comp.id=fr.company_id
WHERE comp.status='closed'
    AND fr.is_first_round = 1
...
SELECT 
  DISTINCT pe.id, ed.instituition
FROM 
  people AS pe 
  JOIN company AS comp ON pe.company_id = comp.id
...
SELECT pe.id, COUNT(ed.instituition) AS num_of_instituitions
FROM people AS pe
JOIN company AS comp ON pe.company_id = comp.id
JOIN education AS ed ON pe.id = ed.person_id
...
SELECT AVG(num_of_instituitions) AS average_num_of_instituitions
FROM (
  SELECT COUNT(ed.instituition) AS num_of_instituitions
  FROM people AS pe
  JOIN company AS comp ON pe.company_id = comp.id
  JOIN education AS ed ON pe.id = ed.person_id
  WHERE pe.company_id IN (
...
WHERE c.milestones > 6 AND fr.funded_at >= '2012-01-01' AND fr.funded_at <= '2013-12-31';
...
SELECT acquiring.name AS acquiring_company,
       ac.price_amount AS deal_amount,
       acquired.name AS acquired_company,
       acquired.funding_total AS investment_amount,
       ROUND(ac.price_amount / acquired.funding_total) AS investment_multiplier
FROM acquisition AS ac
JOIN company AS acquiring ON ac.acquiring_company_id = acquiring.id
JOIN company AS acquired ON ac.acquired_company_id = acquired.id
WHERE ac.price_amount != 0
  AND acquired.funding_total != 0
ORDER BY deal_amount DESC, acquired_company ASC
LIMIT 10;
...
SELECT c.name AS company_name, EXTRACT(MONTH FROM fr.funded_at) AS funding_month
FROM company AS c
...
WITH 
main_acq AS (SELECT  
            EXTRACT(MONTH FROM a.acquired_at) AS round_month,
            COUNT(a.acquired_company_id) AS count_co,
            SUM(a.price_amount) AS summary
    FROM acquisition AS a
    WHERE EXTRACT(YEAR FROM a.acquired_at) IN (2010, 2011, 2012, 2013)
    GROUP BY round_month
            ),
 
main_f AS (SELECT EXTRACT(MONTH FROM fr.funded_at) AS round_month
           , COUNT(DISTINCT f.name) AS count_of_funds
...
SELECT main_acq.round_month,
        main_f.count_of_funds,
        main_acq.count_co,
        main_acq.summary
FROM main_acq
LEFT JOIN main_f ON main_acq.round_month=main_f.round_month;
...
SELECT c.country_code, c.avg_investment_2011, c2.avg_investment_2012, c3.avg_investment_2013
FROM (
    SELECT country_code, AVG(funding_total) AS avg_investment_2011
    FROM company
    WHERE EXTRACT(year FROM founded_at) = 2011
    GROUP BY country_code
) AS c
JOIN (
    SELECT country_code, AVG(funding_total) AS avg_investment_2012
    FROM company
    WHERE EXTRACT(year FROM founded_at) = 2012
    GROUP BY country_code
) AS c2 ON c.country_code = c2.country_code
JOIN (
    SELECT country_code, AVG(funding_total) AS avg_investment_2013
    FROM company
    WHERE EXTRACT(year FROM founded_at) = 2013
    GROUP BY country_code
) AS c3 ON c.country_code = c3.country_code
ORDER BY c.avg_investment_2011 DESC;